...
Because the `GROUP BY time()` clause overrides timestamps, the `ELAPSED()` calculation will always return the same thing: the `GROUP BY time()` interval.

##### Example
<br>
Trying to find the difference between the minimum values at 12-minute intervals.
In the following raw data, the minimum value for the first 12-minute interval occurs at `2015-08-18T00:42:00Z`.
The minimum value for the second 12-minute interval occurs at `2015-08-18T00:48:00Z`.
That's an elapsed time of six minutes between minimum values.

```
> SELECT "water_level" FROM "h2o_feet" WHERE "location" = 'santa_monica' AND time >= '2015-08-18T00:36:00Z' AND time <= '2015-08-18T00:54:00Z'
...
## MOVING_AVERAGE()
Returns the moving average across a `window` of subsequent field values for a single [field](/influxdb/v1.1/concepts/glossary/#field).
...
It then calculates the difference between subsequent field values and
